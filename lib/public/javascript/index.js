...
fetchWeather = (lat, lng, locale) => {
    fetch('/coordinates', {
        method: 'POST',
        mode: 'cors',
        headers: {
            'Content-Type': 'application/json' },
        body: JSON.stringify({lat: lat, lng: lng, locale: locale})
    //    append locale to path
    })
        .then(response => response.json())
        .then(data => displayWeather(data))
}

fetchWeatherCity = (city, locale) => {
    fetch('/weather', {
        method: 'POST',
        mode: 'cors',
        headers: {
            'Content-Type': 'application/json' },
        body: JSON.stringify({city: city, locale: locale})
    })
        .then(response => response.json())
        .then(data => {
            displayWeather(data);
...
function submitWeatherForm(e) {
    e.preventDefault();
    fetchWeatherCity(city.value, locale);
    city.value = ""
...
if (apiData === "") {
    navigator.geolocation.getCurrentPosition((data) => {
        fetchWeather(data.coords.latitude, data.coords.longitude, locale);
...
function onDragEnd() {
    ...
    fetchWeather(lngLat.lat, lngLat.lng, locale);
}
